// useCamera.js — map viewport: offset, zoom, hex coordinate conversion
(function() {
  const { ref } = Vue;

  // Flat-top hex geometry constants
  const HEX_SIZE = 18;                         // center-to-vertex
  const HEX_W    = HEX_SIZE * 2;               // 36px full width
  const HEX_H    = HEX_SIZE * Math.sqrt(3);    // ~31.2px full height
  const HORIZ    = HEX_W * 0.75;               // 27px column spacing
  const VERT     = HEX_H;                      // ~31.2px row spacing

  function useCamera(gridWidth, gridHeight, hexSize) {
    const MIN_ZOOM = 0.5;
    const MAX_ZOOM = 2.0;
    ...
    function centerOn(canvasWidth, canvasHeight) {
      ...
      const z = zoom.value;
      // Center viewport on the middle hex of the grid
    ...
    // Hex center in screen (pixel) space
    ...
    // Screen pixel → hex grid coords (odd-q offset)
    function screenToGrid(sx, sy) {
      const z = zoom.value;
      // Screen → world, subtract grid origin offset (hex 0,0 is at HEX_W/2, HEX_H/2)
      const wx = (sx - offsetX.value) / z - HEX_W / 2;
      const wy = (sy - offsetY.value) / z - HEX_H / 2;

      // World → fractional axial coords (flat-top)
  ...
  window.SpaceColony.useCamera = useCamera;
})();
